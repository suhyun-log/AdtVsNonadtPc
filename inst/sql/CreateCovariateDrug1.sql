 IF OBJECT_ID('AdtVsNonadtPc_@list', 'U') IS NOT NULL
  	...
  	create table @target_database_schema.AdtVsNonadtPc_@list as(
			...
			and c.index_before_1yr <= d.drug_exposure_start_date
			and d.drug_exposure_start_date <= c.index_date
			);
 IF OBJECT_ID('AdtVsNonadtPc_Drug_History_@list', 'U') IS NOT NULL
  	drop 	table @target_database_schema.AdtVsNonadtPc_Drug_History_@list;
	create 	table @target_database_schema.AdtVsNonadtPc_Drug_History_@list as(
